...
pipeline {
  ...
  stages {
    stage('Checkout') {
      steps {
       git branch: 'main', credentialsId: 'GitHubCred', url: 'https://github.com/saiguda654/springboot-build-pipeline.git'
      }
    }
   ...
   stage('Stage VIII: Smoke Test ') {
      steps { 
        ...
        sh "docker run -d --name smokerun -p 8080:8080 guda654/democicd"
